...
local bg, bs, bb = false,false,false;
...
local function ShowCompletedGoals(n1, n2)
	SetWindowVisible(interface, "wnd_goals", false);
	SetWindowVisible(interface, "completed_goals", false);
	if (n1 >= 0)
	then
		SetWindowVisible(interface, "wnd_goals", true);
		SetWindowVisible(interface, "completed_goals", true);

		SetText(interface, "completed", string.format("%0.2d ! %0.2d",n1,n2));
		
		
	else
		SetText(interface, "completed", "");
	end
	UpdateInterfaceBuffers(interface);
end

local function ShowRankTime(n)
	SetWindowVisible(interface, "rank_a", false);
	SetWindowVisible(interface, "rank_b", false);
	SetWindowVisible(interface, "rank_c", false);
	SetWindowVisible(interface, "wnd_rank", false);
	if (n >= 0)
	then
		SetWindowVisible(interface, "wnd_rank", true);
		local m1,s1,hs1 = GetFormattedTime(GetMedalTime(n));
		SetText(interface, "rank_time", string.format("%0.2d:%0.2d:%0.2d",m1,s1,hs1));
...
local function DrawNitro()
	local arm = GetNitro();
	local percent = tonumber(arm) / 100.0 * 23.0;
...
--		SetWindowVisible(interface, "_"..sss, false);
...
--	for i = percent+1,23	do
--		local sss = string.format("_nitro%d", i);
--		SetWindowVisible(interface, sss, true);
...
local function DrawTime()
 	local 	minutes, secs, msecs;
	if (IsTimeMode())
	then
		minutes, secs, msecs = GetTimeAttack(); 
	else
		minutes, secs, msecs = GetTime(); 
	end
...
--			SetText(interface, "nitro", str2); 
  	...
  	local 	minutes2, secs2, msecs2;
		if (IsTimeMode())
		then
			minutes2, secs2, msecs2 = GetBestTimeAttack(); 
		else
			minutes2, secs2, msecs2 = GetTimeBest(); 
		end
...
local bSign3 		= false;
local bSignVisible3	= true;
local function UseRespawn()
--~ 	local pangle, spangle = GetPlayerSplineAngle();
--~ 	local angle = pangle - spangle;
--~ 	while(angle > 6.283)
--~ 	do
...
--~ 	end
	...
	then
		...
		if(bSignVisible3)
		then
			SetWindowVisible(interface, "respawn", false);
		end
		bSign3 = false;
	end
	...
	then
		...
		then
			...
			then
				...
				SetWindowVisible(interface, "respawn", true);
				bSignVisible3 = true;
				dt3 = 0;
			end
		end
...
local function HideResults()
	...
	SetText(interface, "race_rank", "");
	SetText(interface, "race_rank_points", "");
	...
	SetText(interface, "level_unlocked", "");
...
local function UpdateResults()
	...
	rtime = rtime + tt;
	if (not rflag1 and rtime >= 1)
	then
		prevMedal = GetLevelMedal(GetCurrentMission());
		local rank;
		local prevrank;
		if (prevMedal == 4)
		then
		...
		then
			rank = "rank0";
			SetPointsLocal(30);
			SetLevelMedal(GetCurrentMission(), 0);
		elseif (not bs)
		then
			rank = "rank1";
			SetPointsLocal(20);
			SetLevelMedal(GetCurrentMission(), 1);
		elseif (not bb)
		then
			rank = "rank2";
			SetPointsLocal(10);
			SetLevelMedal(GetCurrentMission(), 2);
		else
			rank = "norank";
			SetPointsLocal(0);
		end
		
		if (prevrank == "norank" and rank ~= "norank")
		then
			nextlevel = true;
		else
			nextlevel = false;
		...
		SetText(interface, "race_rank", GetString(prevrank).." > "..GetString(rank));
		rflag1 = true;
		rtime = 0;
--		PlaySpecialSound("fin");
	elseif (rflag1 and not rflag2 and rtime >= 1)
	then
		PlaySpecialSound("fin");
		rtime = 0;
  	rflag2 = true;
		local 	str2 = string.format("%d points", math.max(0, GetPointsLocal() - prevpoints));
		SetText(interface, "race_rank_points", str2);
	elseif (rflag2 and not rflag3 and rtime >= 1)
	then
--		PlaySpecialSound("fin");
		SetTextString(interface, "total", "total_points");
		rflag3 = true;
		rtime = 0;
	elseif (rflag3 and not rflag4 and rtime >= 1)
	then
		PlaySpecialSound("fin");
		rtime = 0;
  	rflag4 = true;
		SetPoints(math.max(0, GetPointsLocal() - prevpoints) + GetPoints());
		local 	str2 = string.format("%0.3d", GetPoints());
		SetText(interface, "total_points", str2);
	elseif (rflag4 and not rflag5 and rtime >= 1)
	then
		rflag5 = true;
		rtime = 0;
		local newcar = false;
		...
		then
			local passed = false;
			if (GetNumRacesPassed() == GetCurrentMission() and GetNumRacesPassed() ~= 16)
			then
				passed = true;
			end
			local n1 = tonumber(GetPoints());
			...
			if (passed)
			then
				SetNumRacesPassed(GetNumRacesPassed() + 1);
				PlaySpecialSound("checkpoint");
				SetTextString(interface, "level_unlocked", "level_unlocked");
			end		
		end
	...
	elseif (rflag5 and not rflag6 and rtime >= 1)
	then
		rflag6 = true;
		rtime = 0;
...
--	SetWindowVisible(interface, "proigrish!_2", false);
	...
	SetWindowVisible(interface, "norank!", false);
	SetWindowVisible(interface, "rank_a!", false);
	SetWindowVisible(interface, "rank_b!", false);
	SetWindowVisible(interface, "rank_c!", false);
	SetWindowVisible(interface, "nearmiss!", false);
	SetWindowVisible(interface, "directhit!", false);
	SetWindowVisible(interface, "traffichit!", false);
	SetWindowVisible(interface, "criminalhit!", false);
	SetWindowVisible(interface, "crash!", false);
...
--		PlaySpecialSound("text");
		...
		then
			...
			SetWindowVisible(interface, GameMessage.wnd, true);
			SaveGame();
		elseif ("norank!" == GameMessage.wnd)
		then
			MAX_TIME = 5;
			game_finish = true;
			SetWindowVisible(interface, GameMessage.wnd, true);
			SaveGame();
		elseif ("crash!" == GameMessage.wnd)
		then
			MAX_TIME = 1;
				SetWindowVisible(interface, GameMessage.wnd, true);
				SetCrash(true);
		...
		then
			...
			SetWindowVisible(interface, GameMessage.wnd, true);
		elseif("rank_a!" == GameMessage.wnd or "rank_b!" == GameMessage.wnd or "rank_c!" == GameMessage.wnd)
		then	
			game_finish = true;
			MAX_TIME = 3;
			SetWindowVisible(interface, GameMessage.wnd, true);
...
--~ 	if("start!" == GameMessage.wnd)
--~ 	then
--~ 			SetWindowSizeAndPos(GameMessage.wnd, 453,61, 350, (0 + (GameMessage.dt / MAX_TIME) * (GameMessage.dt / MAX_TIME) * (GameMessage.dt / MAX_TIME) * 100.0) * 0.1);
--~ 	elseif("ready!" == GameMessage.wnd)
--~ 	then
--~ 			SetWindowSizeAndPos(GameMessage.wnd, 453,61, 350, (0 + (GameMessage.dt / MAX_TIME) * (GameMessage.dt / MAX_TIME) * (GameMessage.dt / MAX_TIME) * 100.0) * 0.1);
--~ 	end
...
--		SetTextString(interface, "drift", "drift");
		SetWindowVisible(interface, "drift", true);
	elseif (statusArrow == 1)
	then
		SetWindowVisible(interface, "wnd_left", false);
...
local function ShowHideElements()
	if (IsTimeMode())
	then
		SetWindowVisible(interface, "wnd_laps", false);
		SetWindowVisible(interface, "wnd_pos", false);
	else
		SetWindowVisible(interface, "wnd_laps", true);
		SetWindowVisible(interface, "wnd_pos", true);
	end
...
--	SetText(interface, "time_best", "BEST:");
	...
	ShowRankTime(0);
	if (Modulo(tonumber(GetCurrentMission()), 4) == 3 or Modulo(tonumber(GetCurrentMission()), 4) == 0)
	then
		local goals, maxgoals = GetGoals();
		ShowCompletedGoals(goals, maxgoals);	
	else
		ShowCompletedGoals(-1,-1);	
	end
	fdt = 0;
	bg, bs, bb = false,false,false;
	lastDH = 0;
	lastNM = 0;
...
local function ShowRank()
	if (not bg)
	then
		SendCommandByName("game_interface", "show_message", "rank_a!");		
	elseif (not bs)
	then
		SendCommandByName("game_interface", "show_message", "rank_b!");		
		
	elseif (not bb)
	then
		SendCommandByName("game_interface", "show_message", "rank_c!");		
		
	else
		SendCommandByName("game_interface", "show_message", "norank!");		
...
local function DirectHit()
	...
	if (n == 0 and lastDH ~= n)
	then
		SendCommandByName("game_interface", "show_message", "directhit!");		
		AddNitro(100);
	end
...
local function NearMiss()
	local n = tonumber(GetCurrentParams(script));
	if (n == 0 and lastNM ~= n and not IsCrash())
	then
		SendCommandByName("game_interface", "show_message", "nearmiss!");	
		AddNitro(30);
	end
...
commandMap["show_rank"]	= ShowRank;
commandMap["direct_hit"]	= DirectHit;
commandMap["near_miss"]	= NearMiss;
...
--~			end
	...
	--	SaveLog("updateinterface3_4");
		...
		then
			if (not bg)
			then
				if (GetTime2() >= GetMedalTime(0))
				then
					bg = true;
					ShowRankTime(1);
					PlaySpecialSound("slow");
				else
					local timediff = GetMedalTime(0) - GetTime2();
					if (timediff <= 3)
					then
						fdt = fdt + GetDeltaTime();
						if (fdt >= 1.0)
						then
							fdt = 0;
							PlaySpecialSound("peek");
						end
					end		
				end
			end
			if (not bs)
			then
				if (GetTime2() >= GetMedalTime(1))
				then
					bs = true;
					ShowRankTime(2);
					PlaySpecialSound("slow");
				else
					local timediff = GetMedalTime(1) - GetTime2();
					if (timediff <= 3)
					then
						fdt = fdt + GetDeltaTime();
						if (fdt >= 1.0)
			...
			if (not bb)
			then
				if (GetTime2() >= GetMedalTime(2))
				then
					bb = true;
					ShowRankTime(-1);
					PlaySpecialSound("slow");
				else
					local timediff = GetMedalTime(2) - GetTime2();
					if (timediff <= 3)
					then
						fdt = fdt + GetDeltaTime();
						if (fdt >= 1.0)
