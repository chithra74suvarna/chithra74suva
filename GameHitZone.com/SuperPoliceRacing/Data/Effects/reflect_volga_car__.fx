...
float4 Ambient = {0.5f, 0.5f, 0.5f, 1.0f};      // Ambient color of the material
float4 Specular = {0.95f, 0.95f, 0.95f, 1.0f};  // Specular color of the material
float  Reflectivity = 0.0f; // Reflectivity of the material
float  Power = 1.0f;
			 ...
			 float3 EnvTex : TEXCOORD3 ) : COLOR0
{
	...
	return float4((MatDiffuse.rgb + fSpecular.rgb) * tcolor.rgb + tex2D(g_smpTrilinear, EnvTex).rgb * Reflectivity * tcolor.a, 1.0f);
