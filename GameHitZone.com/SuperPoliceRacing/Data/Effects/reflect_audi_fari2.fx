...
float4 Diffuse = {0.99f, 0.99f, 0.99f, 1.0f};      // Diffuse color of the material
...
//    oPos = mul( oPos, g_mProj );
    ...
    // Compute lighting
    oDiffuse = 1;//dot( vNormal, -g_vLight);// * Diffuse;// + g_vAmbient * Ambient ;//normalize( g_vLight - oViewPos ) ) * Diffuse;
...
//     float4 fSpecular =  Specular * pow(max(0, dot(R, -ViewPos)), Power);
    ...
    return float4( (float3)( g_vLightColor * ( tex2D( g_samScene, Tex0 ) * (MatDiffuse * Diffuse  + g_vAmbient * Ambient) + fSpecular )
                             + texCUBE( g_samEnv, EnvTex ) * Reflectivity * tex2D( g_samScene, Tex0 )[3] * (MatDiffuse + Ambient / 2.0f)
                           ), 1.0f );
}
