...
float4 Render( float2 Tex : TEXCOORD0 ) : COLOR0
{
    float4 vWeightsBW=float4(0.45,0.885,0.165,0);
    float4 vWeightsSepia=float4(1.0,0.8,0.6,1); 
//    float4 vWeightsSepia=float4(0.6,0.8,1,1); 
   float4 cColor=tex2D( g_samSrcColor, Tex ); 
   
   float4 cColor2 = cColor;

    cColor *= fMiddleGray / ( Luminance + 0.001f );
    cColor *= ( 1.0f + ( cColor / ( fWhiteCutoff * fWhiteCutoff ) ) );
    float cnst = 3;
    cColor -= cnst;

    cColor = max( cColor, 0.0f );

    cColor /= ( cnst + cColor );
    
    cColor = 0.1 * cColor + 0.9 * cColor2;
  
    //float  brightness=dot(cColor,vWeightsBW);  
    return cColor2;//brightness*vWeightsSepia * 0.4 + cColor*0.6;
}
